...
interface PassportSidebarDetailsProps {
  passport: Passport
  onEdit: () => void
}

export default function PassportSidebarDetails({ passport, onEdit }: PassportSidebarDetailsProps) {
  const {data} = useQuery({
    queryKey: ["admin", "passports", passport.passport_id],
    queryFn: () => PassportApi.get(passport.passport_id)
  })
